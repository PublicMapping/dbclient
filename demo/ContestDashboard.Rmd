              ...
              sidebarLayout(
                ...
                mainPanel(
                  tabsetPanel(type = "tabs",
                              tabPanel("Score Tradeoffs ", plotlyOutput("Tradeoffs"),textOutput("Selection")),
                              tabPanel("Score Distributions", plotOutput("Distributions")) # TODO: Stacking Plotly output in Shiny
                  ),
          ...
          server = function(input, output, session) {
            ...
            filtered.tb <- reactive({ 
              if (input$filterLegal) {
                scores.tb %>% filter(unassigned_geography==FALSE, 
                                     unassigned_geography == FALSE,
                                     plan_region==input$region)
              } else {
                scores.tb %>% filter(plan_region==input$region)
              }
            })
            
            
            output$Tradeoffs <- renderPlotly({
              p1 <- filtered.tb() %>% 
            ...
            output$Distributions <- renderPlot({
              ...
              # TODO: test using plotly subplots instead of patchwork: https://plotly.com/r/subplots/
              pt <- p1 / p2
              pt
            })
  ...
  ui <- fluidPage(
    ...
    sidebarLayout(
      ...
      sidebarPanel(
        ...
        #       a(href="",target="_db","See plan in DistrictBuilder")
        #   )
        # })
      ...
      mainPanel(
        ...
        tabsetPanel(type = "tabs", id="tabs",
                    tabPanel(value="single", "Plan Map", plotOutput("map")),
                    tabPanel(value="single", "Plan Overview", tableOutput("overview")),
                    tabPanel(value="single", "Plan Districts", DTOutput("details")),
                    tabPanel(value="comparison", "Map Comparison", plotOutput("compareMap")),
                    tabPanel(value="comparison", "Score Comparison", tableOutput("compareTable"))
        )
      )
    )
  ),
