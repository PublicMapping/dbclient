...
thematic_shiny()
 ...
 contestPlans.tb %>% 
  select(-map,-districtScoreTable,-file_src,-planscoreURI,-description) %>%
   relocate(plan_name,url,unassigned_geography,unassigned_population,numdists,contiguity_mn,majMin_n,county_split_count,county_split_sum) %>%

  #mutate(description = str_trunc(str_flatten(str_squish(description)),100)) %>%
  mutate(url=paste("<a href='",url,"' target='_db'>",url,"</a>",sep="")) %>%
  datatable(class = "cell-border stripe; hover; .scrolledTable", caption = "",
            width="100%",
            editable=FALSE,
            escape=FALSE,
            selection="none",
